...
const WelcomeLayout = ({children,title,handleProceed,buttonTitle, index, skip, skipNextScreen, showButton, scrollable = true}:iWelcomeLayout) => {
    ...
    <View className="bg-[#FFFFFF] flex-1 rounded-t-2xl pt-[60px] px-[38px] h-full justify-between items-center relative">
        {scrollable ? 
            (<ScrollView className="flex-1 w-full" showsVerticalScrollIndicator={false}>
                ...
                <Text style={{fontFamily:'geometria-bold'}} className="text-main-blue text-center text-lg leading-5 mb-10">
                    {title}
                </Text>
                {children}
                </View>
